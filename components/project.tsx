...
export default function Project({
  ...
  return (
    ...
    >
      <section className=" relative bg-gray-100 dark:bg-white/10 dark:hover:bg-white/20 max-w-[45rem] border border-black/5 overflow-hidden sm:pr-8 sm:h-[20rem] sm:group-even:pl-8 rounded-lg   hover:bg-gray-200 transition">
        <div className="flex flex-col pt-4 pb-7 px-5 sm:pl-10 sm:pr-2 sm:pt-10 sm:max-w-[50%] h-full sm:group-even:ml-[18rem]">
          ...
          <ul className=" flex flex-wrap mt-4 gap-2 sm:mt-auto">
            {tags.map((tag, index) => (
              <li
                key={index}
                className=" bg-black/[0.7] px-3 py-1 text-[0.7rem] text-white dark:text-white/70 uppercase tracking-wider rounded-full"
              >
                {tag}
              </li>
            ))}
